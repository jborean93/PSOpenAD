...
Describe "Remove-OpenADGroupMember cmdlet" -Skip:(-not $PSOpenADSettings.Server) {
    BeforeAll {
    ...
    Context "Remove-OpenADGroupMember" {
        ...
        It "Removes group member through pipeline input" {
            $group | Remove-OpenADGroupMember -Session $session -Members $contact
        ...
        It "Runs with -PassThru" {
            $actual1 = $group | Remove-OpenADGroupMember -Session $session -Members $contact -PassThru
        ...
        It "Runs with -WhatIf and -PassThru" {
            $actual1 = $group | Remove-OpenADGroupMember -Session $session -Members $contact -WhatIf -PassThru
            $actual2 = $group | Get-OpenADObject -Session $session -Property member
