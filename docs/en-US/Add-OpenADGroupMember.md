...
online version: https://www.github.com/jborean93/PSOpenAD/blob/main/docs/en-US/Add-OpenADGroupMember.md
schema: 2.0.0
---
...
### Server (Default)
```
Add-OpenADGroupMember [-Identity] <ADPrincipalIdentity> [-Members] <ADPrincipalIdentity[]> [-PassThru]
...
## DESCRIPTION
The `Add-OpenADGroupMember` cmdlet adds one or more users, groups, service accounts, or computers to an Active Directory group.

The `Identity` parameter specifies the Active Directory group to add members to.
...
The `Members` parameter specifies the users, groups, service accounts, and computers to add to the group.
...
### Example 1: Add members to a group
```powershell
PS C:\> Add-OpenADGroupMember -Identity 'Group0' -Member 'CN=User0,OU=Factory,DC=domain,DC=test'
```

Adds the object `CN=User0,OU=Factory,DC=domain,DC=test` to the group `Group0`.

### Example 2: Add members to piped in group
```powershell
PS C:\> Get-OpenADGroup -Identity 'CN=Group0,OU=Factory,DC=domain,DC=test' | Add-OpenADGroupMember -Member 'User0'
```

Adds the object `User0` to the group `CN=Group0,OU=Factory,DC=domain,DC=test`.

### Example 3: Add multiple members to a group
```powershell
PS C:\> $members = @(
...
PS C:\> Add-OpenADGroupMember -Identity 'Group0' -Member $members
```

Adds the objects specified by the `$members` variable to the group `Group0`.
...
### -Members
Specifies an array of users, groups, service accounts, and computers in a comma-separated list to add to a group using one of the following formats:
